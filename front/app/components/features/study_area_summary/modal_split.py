...
def ModalSplitList(
    ...
    rows = [
        _row("Car", share_car),
        _row("Cycling", share_bike),
        _row("Walking", share_walk),
        _row("Carpooling", share_carpool),
    ]
    ...
    if (share_pt or 0) > 0:
        rows.append(
            dmc.Group(
                [
                    dmc.Text("Public Transport", fw=700, size="sm"),
        ...
        # Sous-modes (indentés)
        sub = [
            _row("  Walk + PT", share_pt_walk),
            _row("  Car + PT", share_pt_car),
            _row("  Bike + PT", share_pt_bicycle),
        ]
